          ...
          if (!botchannel.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
            ...
            return message.reply({
              embeds: [new Discord.MessageEmbed()
                ...
                .setDescription(`There is a Bot chat setup in this GUILD! Try using the Bot Commands here: ${botchannel.map(c=>`<#${c}>`).join(", ")}`)
              ]
            }).then(async msg => {
            ...
            if (!premiumdata.guilds.includes(message.guild.id)) {
              return message.reply({
                embeds: [
                  ...
                  .setDescription(`An Admin can request this Server as PREMIUM-GUILD by typing: ${prefix}premium`)
                ]
              })
            }
